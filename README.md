...
Add these as you capture them. Save images into a `screenshots/` folder next to this README, then reference them like this:
...
|---|---|---|---|
| 1 | Azure VM Overview page (shows name, IP, region, size, status) | Azure Portal → Virtual Machines → IronForge-VM → Overview tab | End of **Phase 1** |
| 2 | Networking / NSG inbound rules (22, 80, 443 visible) | Azure Portal → your VM → Networking tab | End of **Phase 1** |
| 3 | Terminal showing successful SSH connection | Your terminal, right after `ssh <user>@74.162.66.196` | Start of **Phase 2** |
| 4 | Terminal showing `apt update && apt upgrade` output | Your terminal after running the update command | End of **Phase 2** |
| 5 | `sudo systemctl status nginx` output showing "active (running)" | Your terminal after installing/starting nginx | End of **Phase 3** |
| 6 | Browser showing nginx default welcome page (before file upload) | Browser at `http://74.162.66.196` | End of **Phase 3** |
| 7 | SFTP/SCP client mid-transfer (files uploading to `/var/www/html`) | FileZilla/WinSCP window, or terminal `scp` output | Start of **Phase 4** |
| 8 | Browser showing your live homepage | Browser at `http://74.162.66.196` | End of **Phase 4** |
| 9 | Browser showing each inner page (equipment, membership, schedule, trainers, contact) | Browser, one screenshot per page | End of **Phase 4** |
| 10 | DNS A record settings (only if you set up a domain) | Your domain registrar's DNS management panel | **Phase 5** |
| 11 | Terminal showing `certbot` cert issued successfully (only if HTTPS set up) | Your terminal after running certbot | **Phase 6** |
| 12 | Browser showing the padlock/HTTPS on your domain (only if applicable) | Browser address bar | End of **Phase 6** |

Tip: crop screenshots to just the relevant panel (not your whole desktop) and keep filenames descriptive (`vm-overview.png`, not `Screenshot 2026-07-31.png`) so they're easy to drop into the Word doc or slides too if your assignment needs those.
